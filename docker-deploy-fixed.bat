@echo off
echo ==========================================
echo    AnythingLLM - Fixed Docker Deployment
echo ==========================================
echo.
...
echo [3/5] Building updated Docker image with chat widget fix...
...
echo [5/5] Starting container with fixed configuration...
docker run -d ^
...
powershell -Command "try { $response = Invoke-WebRequest -Uri 'http://localhost:3001/api/ping' -TimeoutSec 10 -UseBasicParsing; Write-Host 'SUCCESS: Application is responding!' } catch { Write-Host 'FAILED: Application not responding yet' }"
...
echo   Chat Widget Test: http://localhost:3001/embed/test-widget.html
echo.
